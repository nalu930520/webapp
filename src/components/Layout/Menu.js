...
const Menus = ({ siderFold, darkTheme, handleClickNavMenu, navOpenKeys, changeOpenKeys, menu }) => {
    ...
    return menuTreeN.map(item => {
        ...
        return (
          <Menu.ItemGroup
            ...
            title={<span>
              ...
              {(!siderFoldN || !menuTree.includes(item)) && <FormattedMessage id={item.name}/>}
            </span>}
          >
      ...
      return (
        <Menu.Item key={item.id}>
          {item.redirect ? <a href={item.route} target={item.id = 9 ? '_self' : '_blank'} onClick={(e) => {e.stopPropagation()}}>
  ...
  if (location.pathname.indexOf('myWallets/transaction') > -1) {
    defaultSelectedKeys = ['1','2'];
  }
  if (location.pathname.indexOf('sendreceive') > -1) {
